...
public class RevealBackgroundView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        if (state == STATE_FINISHED) {
            ...
            canvas.drawCircle(startLocationX, startLocationY, currentRadius, fillPaint);//画圆
        }
    }
    ...
    private void changeState(int state) {
        ...
        if (onStateChangeListener != null) {
            onStateChangeListener.onStateChange(state);//如果调用了changeState方法，发出事件信号通知stateChanged
        }
    }
    ...
    public void setCurrentRadius(int radius) {//ObjectAnimator内部的工作机制是通过寻找特定属性的get和set方法
